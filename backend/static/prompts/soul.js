(function() {
    ...
    // 所有魂穿 prompt 共用：嚴禁現代網文/評論語彙
    const MODERN_VOCAB_BAN = `【語言禁令】嚴禁在正文、分析或報告中使用以下現代網文/評論語彙：降維打擊、極限微操、微操、控場、碾壓、開掛、爽點、道德高地、道德綁架、系統性崩潰、系統性癱瘓、新手村、血包、打工人、劇本（比喻設計感）、降維、跨維度。請改用古代或中性語彙：制馭、籌謀、臨機處斷、威服、壓服、彈壓、折衝、恩威並施、牽制、勢不得不然、分化拉攏、因勢利導。`;

    // 史書編年模式追加：禁分析師語氣 + 要求關鍵一幕多樣
    const CHRONICLE_STYLE_BAN = `【史書文體禁令】正文與沙盤報告嚴禁「分析師語氣」：結構性、系統性、策略框架、模式、完美微操、打出優勢、賦能、顆粒度。史官可評可議，但須用史家語氣——例如「其患不在一人」「財用日絀」「邊餉不繼」「群臣觀望」「天子倦勤」「民怨已積」「衰象已兆」「勝負之數難料」。
【關鍵一幕多樣性】不得以「近身壓迫／人格震攝」為唯一關鍵一幕類型。請按歷史邏輯選擇，並在各章輪替：廷議爭論、奏疏往來、軍報傳遞、詔書頒布、幕僚私議、邊鎮急報、地方民變、外交折衝、宮廷暗流、賦稅財政危機、繼承人角力。`;
...
請輸出精煉後的全局主線摘要：`;
    ...
    window.buildSoulChronicleChapterPrompt = function({ soulSession, chNum, label, host, soul, hostMod, soulMod, factionStates, figureStates, globalSummary, pendingTensions, keyEvents, totalMonthsElapsed, chronicleScale }) {
        ...
        const scaleWordCount = { fine: '1200-1800', standard: '1600-2200', epic: '1800-2600' }[chronicleScale] || '1600-2200';
...
本章正文總字數目標：${scaleWordCount}字。請充分展開，不得因提供四段結構而縮水。

請按以下四段結構輸出正文：

**一、時代走筆**（700-1000字）
以編年史筆法記錄本時期整體局勢：政治格局、軍事動向、民心向背、關鍵轉捩。保持史家的冷靜全景視角，提煉時代面貌而非逐日流水帳。史官應有明確的史評立場，而非中立旁觀的播報員。歷史演化的複雜性、不可預料性、人物各自的利益邏輯都應有所體現。

**二、關鍵場景**（400-700字，可寫1-2場）
從這段時間中選取最具代表性的時刻，以場景還原方式充分展開對話、動作與氣氛。人物語言須符合時代感，不得以現代口語替代。可寫1場深入或2場相互呼應的場景，不得草草帶過。場景類型請從以下輪替，不得連章重複同一類型：廷議爭論、奏疏往來、軍報傳遞、詔書頒布、幕僚私議、邊鎮急報、地方民變、外交折衝、宮廷暗流、賦稅財政危機。

**三、沙盤報告**（200-350字）
逐條列出本時期各主要勢力消長，每條須說明趨勢根因，不只是結果陳述：
格式：【勢力名稱】趨勢（↑/↓/→）　當前狀態　代表性轉折或事件及其成因

**四、史官評語 · 靈魂旁白**（150-250字）
前半以史官語氣對本時期作總評，點出歷史的諷刺、必然或意外；後半以穿越靈魂的視角評述這段歷史——靈魂帶來了什麼影響，或感受到什麼無力。兩者可以呼應，也可以形成對照張力。
